...
- **Type-check before push:** Run `npx tsc --noEmit` (or equivalent) before pushing code to any remote. A push with type errors wastes a full deploy cycle per error. Three consecutive failures in one session is the sign this gate was missing. (Field report #299)
