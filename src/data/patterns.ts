...
module TenantContext
  ...
  },
  {
    ...
    description: "Branded Cents type, hash-chained append log, atomic writes, per-project path resolution.",
    teaches: "How to handle money safely: branded Cents type (never raw numbers), append-only transaction log with hash chaining for integrity, atomic file writes to prevent corruption, and per-project treasury path resolution (v22.0).",
    ...
    frameworks: [{ framework: "typescript", label: "TypeScript", language: "TypeScript", code: `type Cents = number & { __brand: "cents" };\n\nfunction toCents(dollars: number): Cents {\n  return Math.round(dollars * 100) as Cents;\n}\n\nfunction toDisplay(cents: Cents): string {\n  return \`$\${(cents / 100).toFixed(2)}\`;\n}\n\ninterface Transaction {\n  id: string;\n  amount: Cents;\n  type: "revenue" | "spend" | "transfer";\n  timestamp: string;\n  prevHash: string;\n  hash: string;\n}\n\n// ─── Project-Scoped Paths (v22.0) ────────────────────────────\n// Global treasury paths (default — used by vault, TOTP, backup)\nconst TREASURY_DIR = join(homedir(), ".voidforge", "treasury");\nconst SPEND_LOG = join(TREASURY_DIR, "spend-log.jsonl");\nconst REVENUE_LOG = join(TREASURY_DIR, "revenue-log.jsonl");\nconst PENDING_OPS = join(TREASURY_DIR, "pending-ops.jsonl");\nconst BUDGETS_FILE = join(TREASURY_DIR, "budgets.json");\n\n// Per-project paths — pass projectDir for project isolation.\n// When projectDir is provided: {projectDir}/cultivation/treasury/\n// When omitted: falls back to global ~/.voidforge/treasury/\n// Vault (vault.enc) and TOTP (totp.enc) stay global —\n// credentials are user-scoped, not project-scoped (ADR-040 §4).\n// Replaces 4 hardcoded TREASURY_DIR definitions across\n// financial-vault.ts, totp.ts, treasury-backup.ts.\nfunction getTreasuryDir(projectDir?: string): string {\n  if (projectDir) return join(projectDir, "cultivation", "treasury");\n  return TREASURY_DIR;\n}\n\nfunction getSpendLog(projectDir?: string): string {\n  return join(getTreasuryDir(projectDir), "spend-log.jsonl");\n}\n\nfunction getRevenueLog(projectDir?: string): string {\n  return join(getTreasuryDir(projectDir), "revenue-log.jsonl");\n}\n\nfunction getPendingOps(projectDir?: string): string {\n  return join(getTreasuryDir(projectDir), "pending-ops.jsonl");\n}\n\nfunction getBudgetsFile(projectDir?: string): string {\n  return join(getTreasuryDir(projectDir), "budgets.json");\n}\n\n// The heartbeat daemon uses active*() wrapper functions\n// that call these with the daemon's configured projectDir.\n\nexport {\n  toCents, toDisplay,\n  getTreasuryDir, getSpendLog, getRevenueLog,\n  getPendingOps, getBudgetsFile,\n};` }],
  },
  {
    slug: "daemon-process",
    name: "daemon-process.ts",
    title: "Daemon Process",
    description: "PID management, Unix socket API, job scheduler, signal handling, per-project configuration.",
    teaches: "How to build a long-running daemon with PID file management, Unix socket control API, cron-like job scheduling, graceful signal handling, and per-project path configuration with dual-daemon guard (v22.0).",
    ...
    frameworks: [{ framework: "typescript", label: "TypeScript", language: "TypeScript", code: `import fs from "fs";\nimport net from "net";\n\nclass Daemon {\n  private pidFile: string;\n  private socketPath: string;\n\n  async start() {\n    if (this.isRunning()) throw new Error("Already running");\n    this.writePid();\n    this.startSocket();\n    process.on("SIGTERM", () => this.shutdown());\n    process.on("SIGINT", () => this.shutdown());\n  }\n\n  private writePid() {\n    fs.writeFileSync(this.pidFile, String(process.pid));\n  }\n\n  private startSocket() {\n    const server = net.createServer((conn) => {\n      conn.on("data", (data) => this.handleCommand(data.toString()));\n    });\n    server.listen(this.socketPath);\n  }\n\n  async shutdown() {\n    fs.unlinkSync(this.pidFile);\n    fs.unlinkSync(this.socketPath);\n    process.exit(0);\n  }\n}\n\n// ─── Per-Project Configuration (v22.0) ───────────────────────\n// Default paths (global — ~/.voidforge/run/)\n// Path variables are let (mutable) — configurePaths() overrides\n// them before daemon startup.\nlet PID_FILE = join(RUN_DIR, "heartbeat.pid");\nlet SOCKET_PATH = join(RUN_DIR, "heartbeat.sock");\nlet TOKEN_FILE = join(VOIDFORGE_DIR, "heartbeat.token");\nlet STATE_FILE = join(VOIDFORGE_DIR, "heartbeat.json");\nlet LOG_FILE = join(VOIDFORGE_DIR, "heartbeat.log");\n\n// Preserved as const for the dual-daemon guard check\nconst GLOBAL_PID_FILE = join(RUN_DIR, "heartbeat.pid");\n\n// Override for per-project daemon.\n// Call BEFORE writePidFile/createSocketServer.\n// CLI: voidforge heartbeat start --project-dir /path/to/project\nfunction configurePaths(projectDir: string): void {\n  const cultDir = join(projectDir, "cultivation");\n  PID_FILE = join(cultDir, "heartbeat.pid");\n  SOCKET_PATH = join(cultDir, "heartbeat.sock");\n  TOKEN_FILE = join(cultDir, "heartbeat.token");\n  STATE_FILE = join(cultDir, "heartbeat.json");\n  LOG_FILE = join(cultDir, "heartbeat.log");\n}\n\n// Dual-daemon guard — prevents split-brain financial writes.\n// Per-project daemon refuses to start if a global daemon is running.\nasync function checkGlobalDaemon(): Promise<boolean> {\n  try {\n    const pid = parseInt(fs.readFileSync(GLOBAL_PID_FILE, "utf8"));\n    process.kill(pid, 0); // throws if not running\n    return true;\n  } catch {\n    return false;\n  }\n}\n\nexport { Daemon, configurePaths, checkGlobalDaemon };` }],
  },
  {
