...
export const commands: Command[] = [
  {
    ...
    whatHappens: [
      ...
      "Wong loads cross-project lessons and project-scoped operational learnings",
      "Stark + Kusanagi scaffold the project (signature-first, stored value rename check, worker env verification)",
      "Phases execute in sequence with verification gates",
      "Batman, Galadriel, and Kenobi run the double-pass review cycle",
      "Build artifact verification ensures compiled output matches source",
  ...
  },
  {
    ...
    usage: "/qa",
    whatHappens: [
      "Dispatch-first: Batman's team runs as parallel sub-agents for codebases >10 files",
      "Load operational learnings scoped to components under investigation",
      "Oracle analyzes test coverage + ID Space Audit (verify identifier comparisons use the same ID space)",
      "Red Hood probes error paths",
      "Data-UI enum consistency — verify backend enums match frontend display values exactly",
  ...
  },
  {
    ...
    whatHappens: [
      ...
      "Ahsoka checks read-operation guards — reads leak data too, not just writes",
      "Safety parameter audit — verify critical params (max positions, price floors, rate limits) can't be overridden to unsafe values",
      "CSP execution verification — verify scripts actually run under the policy, not just that headers exist",
  ...
  },
  {
    ...
    usage: "/architect",
    whatHappens: [
      "Load operational learnings — prior decisions and known constraints prevent re-evaluation of settled questions",
  ...
  },
  {
    ...
    whatHappens: [
      ...
      "Crossfire: agents challenge each other's findings — Maul's re-probe of fixes is a mandatory gate",
  ...
  },
  {
    ...
    whatHappens: [
      ...
      "Odo verifies prerequisites + cross-mission data handoff check (closed loops must have explicit wiring)",
      "Scope-based review scaling: S (<100 lines), M (100-300), L (300+) — L missions get 2 review rounds minimum",
      "Per-mission Kenobi quick-scan on any auth/crypto/credential code (don't defer security to Victory Gauntlet)",
      "Sisko presents the mission brief and waits for confirmation",
      "Fury's /assemble runs for the scoped mission (dispatched to sub-agents per Parallel Agent Standard)",
      "Coulson commits and versions the completed mission",
      "Loop: back to Step 1 until every PRD section is implemented",
      "Victory Gauntlet is a hard gate — launches immediately after the final mission, never skipped",
    ],
    arguments: [
      {
        flag: "--blitz",
  ...
  },
  {
    ...
    whatHappens: [
      ...
      "O'Brien + Nog extract operational learnings — API quirks, decision rationale, root causes that took multiple attempts",
      "Approved learnings are written to docs/LEARNINGS.md (50-entry cap, staleness tracking, promotion pipeline)",
  ...
  },
  {
    ...
    whatHappens: [
      ...
      "Round 2: Domain audits — QA, UX, Security, Architecture, DevOps + Vin (statistical review for z-tests, A/B eval, confidence intervals)",
      "Round 3: Adversarial — agents attack each other's domains. Maul's re-probe of fixed areas is a mandatory gate.",
      "Round 4: Crossfire — agents challenge each other's findings with semantic verification (check meaning, not just types)",
  ...
  },
  {
    ...
    whatHappens: [
      ...
      "Operational learnings sync — catches undiscovered API quirks, decision rationale, and root causes even if /debrief wasn't run",
