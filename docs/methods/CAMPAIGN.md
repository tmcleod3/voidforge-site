...
6. **Classify every PRD requirement by type:**
   ...
   - **ToS Verification** — for each external service integration, confirm the provider's Terms of Service permit the intended usage (automated purchasing, API-driven operations, bot-initiated transactions). A ToS violation discovered post-build forces architectural rework. (Field report #300)
